...
if (bUseCustomMoltProbility==FIXED_PROB_MOLT)
 ...
 WriteCtl(catch_emphasis);
  Penalty_emphasis.allocate(1,8,"Penalty_emphasis");
...
ECHO(nYparams);
 ...
 ECHO(log_add_cv);
  ...
  nloglike.allocate(1,nlikes,1,ilike_vector,"nloglike");
    ...
    nloglike.initialize();
  #endif
  nlogPenalty.allocate(1,8,"nlogPenalty");
  #ifndef NO_AD_INITIALIZE
  ...
  #endif
  M.allocate(1,nsex,syr,nyrRetro,1,nclass,"M");
  #ifndef NO_AD_INITIALIZE
    M.initialize();
  #endif
  ...
  F2.allocate(1,nsex,syr,nyrRetro,1,nseason,1,nclass,"F2");
    ...
    F2.initialize();
  #endif
  Z.allocate(1,nsex,syr,nyrRetro,1,nseason,1,nclass,"Z");
  #ifndef NO_AD_INITIALIZE
    Z.initialize();
  #endif
  Z2.allocate(1,nsex,syr,nyrRetro,1,nseason,1,nclass,"Z2");
  #ifndef NO_AD_INITIALIZE
    Z2.initialize();
  #endif
  S.allocate(1,nsex,syr,nyrRetro,1,nseason,1,nclass,1,nclass,"S");
  #ifndef NO_AD_INITIALIZE
    S.initialize();
  #endif
...
void model_parameters::calc_objective_function(void)
{
  ...
  if (nsex > 1)
   ...
   if (bUseCustomMoltProbility==FREE_PROB_MOLT)
   {
    // for ( int h = 1; h <= nsex; h++ )
     // for (int igrow=1;igrow<=nMoltVaries(h);igrow++)
	  // nlogPenalty(8) += dnorm(first_difference(molt_probability_in(h,igrow)), 1.0);  
...
dvar_vector model_parameters::calc_predicted_project(const int YrRef, dvar3_array d4_Npass, dvar3_array _ft, dvar3_array _Z1, dvar3_array _Z2)
{
 {
